...
func (r *PostgresDB) GetAccountByID(ctx context.Context, model *pb.GetAccountByIdRequest) (*ent.Account, *ent.Customer, error){
	...
	log.Println("AAAAAAAAAAAAAAAAAAAAAAAAAA REPO",res1)
	log.Println("AAAAAAAAAAAAAAAAAAAAAAAAAA REPO",res2)
...
func (r *PostgresDB) CreateAccount(ctx context.Context, model *pb.CreateAccountRequest) (*ent.Account, error) {
	res, err := r.Client.Account.Create().SetEmail(model.Email).SetPassword(model.Password).SetRole(account.Role(model.Role.String())).SetAccOwnerID(uuid.MustParse(model.AccOwner)).Save(ctx)
	if err != nil{
		return nil, err
	}
	return res, nil
