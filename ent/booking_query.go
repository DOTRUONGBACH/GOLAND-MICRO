...
type BookingQuery struct {
	...
	withBookingOwner  *CustomerQuery
	withFKs           bool
...
func (bq *BookingQuery) QueryBookingOwner() *CustomerQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, booking.BookingOwnerTable, booking.BookingOwnerColumn),
		)
		fromU = sqlgraph.SetNeighbors(bq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (bq *BookingQuery) Clone() *BookingQuery {
	...
	return &BookingQuery{
		...
		withBookingOwner:  bq.withBookingOwner.Clone(),
...
func (bq *BookingQuery) WithBookingOwner(opts ...func(*CustomerQuery)) *BookingQuery {
	query := &CustomerQuery{config: bq.config}
	for _, opt := range opts {
		opt(query)
	}
	bq.withBookingOwner = query
	return bq
}
...
//		GroupBy(booking.FieldTotalCost).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (bq *BookingQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Booking, error) {
	var (
		...
		loadedTypes = [2]bool{
			...
			bq.withBookingOwner != nil,
		}
	)
	if bq.withBookingFlight != nil || bq.withBookingOwner != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := bq.withBookingOwner; query != nil {
		if err := bq.loadBookingOwner(ctx, query, nodes, nil,
			func(n *Booking, e *Customer) { n.Edges.BookingOwner = e }); err != nil {
			return nil, err
		}
	}
...
func (bq *BookingQuery) loadBookingFlight(ctx context.Context, query *FlightQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Flight)) error {
	...
	for i := range nodes {
		if nodes[i].flight_belongs_to == nil {
			continue
		}
		fk := *nodes[i].flight_belongs_to
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "flight_belongs_to" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (bq *BookingQuery) loadBookingOwner(ctx context.Context, query *CustomerQuery, nodes []*Booking, init func(*Booking), assign func(*Booking, *Customer)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Booking)
	for i := range nodes {
