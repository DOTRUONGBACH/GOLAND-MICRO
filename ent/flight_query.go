...
func (fq *FlightQuery) loadBelongsTo(ctx context.Context, query *BookingQuery, nodes []*Flight, init func(*Flight), assign func(*Flight, *Booking)) error {
	...
	for _, n := range neighbors {
		fk := n.flight_belongs_to
		if fk == nil {
			return fmt.Errorf(`foreign-key "flight_belongs_to" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "flight_belongs_to" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
