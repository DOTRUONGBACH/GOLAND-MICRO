...
type Flight struct {
	...
	// AvailableSeat holds the value of the "available_seat" field.
	AvailableSeat int `json:"available_seat,omitempty"`
...
func (*Flight) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case flight.FieldDuration, flight.FieldCapacity, flight.FieldAvailableSeat:
...
func (f *Flight) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case flight.FieldAvailableSeat:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for field available_seat", values[i])
			} else if value.Valid {
				f.AvailableSeat = int(value.Int64)
			}
...
func (f *Flight) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("available_seat=")
	builder.WriteString(fmt.Sprintf("%v", f.AvailableSeat))
	builder.WriteString(", ")
