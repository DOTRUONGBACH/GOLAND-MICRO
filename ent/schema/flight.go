...
func (Flight) Fields() []ent.Field {
	return []ent.Field{
		...
		field.Int("capacity").Positive().Default(50).Annotations(entgql.OrderField("CAPACITY"), entproto.Field(8)),
		field.Int("available_seat").Positive().Default(50).Annotations(entgql.OrderField("AVAILABLE_SEAT"), entproto.Field(9)),
		field.Enum("status").Values("Scheduled", "Delayed", "Departed", "Arrived", "Cancelled" ).Annotations(entgql.OrderField("STATUS"), entproto.Field(10)),
		field.Time("created_at").Default(time.Now).Immutable().Annotations(entgql.OrderField("CREATED_AT"), entproto.Field(11)),
		field.Time("updated_at").Default(time.Now).UpdateDefault(time.Now).Annotations(entgql.OrderField("UPDATED_AT"), entproto.Field(12)),
	}
}
